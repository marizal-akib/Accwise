...
import { CallbackFormScrollHandler } from "@/components/CallbackFormScrollHandler";
import { LeadForm } from "@/components/LeadForm";
import { LiquidButtonLink } from "@/components/ui/liquid-glass-button";
import { HeroContentReveal, RevealGroup, RevealItem } from "@/components/ui/reveal-motion";
  ...
  return (
    <>
        ...
        <div className="relative mx-auto max-w-4xl">
          <HeroContentReveal>
            ...
            <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
              <LiquidButtonLink href="#callback-form" showArrow>
                Request a callback
              </LiquidButtonLink>
              <LiquidButtonLink
                href={`mailto:${contactDetails.email}`}
      ...
      <section className="bg-white">
        <RevealGroup className="mx-auto grid w-full max-w-6xl gap-12 px-5 py-20 sm:px-6 lg:grid-cols-[0.86fr_1.14fr] lg:px-8 lg:py-24">
          <RevealItem className="lg:sticky lg:top-28 lg:self-start">
            <div>
          ...
          <RevealGroup className="grid gap-6" amount={0.18} staggerDelay={0.07}>
            <RevealItem>
              ...
              {contactRoutes.map((route) => {
                ...
                return (
                  <RevealItem key={route.title}>
                    <article
                      className="group relative overflow-hidden rounded-lg border border-accwise-blue/10 bg-white p-7 shadow-[0_24px_70px_rgba(22,37,66,0.09)] transition duration-300 hover:-translate-y-1 hover:border-accwise-green/24 hover:shadow-[0_30px_90px_rgba(22,37,66,0.14)]"
                    >
                      <span
                        aria-hidden="true"
                        className="pointer-events-none absolute inset-y-0 left-[-40%] w-1/3 -skew-x-12 bg-gradient-to-r from-transparent via-white/55 to-transparent opacity-0 transition-all duration-700 group-hover:left-[120%] group-hover:opacity-100 motion-reduce:hidden"
                      />
                      <div className="relative z-10">
                        <Icon
                          aria-hidden="true"
                          className="size-8 text-accwise-green transition duration-300 group-hover:text-accwise-blue"
                          strokeWidth={1.8}
                        />
      ...
      <section className="scroll-mt-24 bg-[#f3f5f8]" id="callback-form">
        <RevealGroup className="mx-auto grid w-full max-w-6xl gap-12 px-5 pt-20 pb-36 sm:px-6 lg:grid-cols-[0.85fr_1fr] lg:px-8 lg:pt-24 lg:pb-40">
          <RevealItem>
